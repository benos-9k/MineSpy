 ...
 * 2012 January 19
 * 
 * Code modified to throw IOException instead of suppressing, Javadoc corrected, toString method added.
 * Also added code to parse from filename and report region coords.
 * - Ben Allen
 * 
 */
...
/**
 * Interfaces with region files on the disk. <br>
 * <br>
 * <b>Region File Format</b>
 * <p>
 * Concept: The minimum unit of storage on hard drives is 4KB. 90% of Minecraft chunks are smaller than 4KB. 99% are
 * smaller than 8KB. Write a simple container to store chunks in single files in runs of 4KB sectors.
 * </p>
 * <p>
 * Each region file represents a 32x32 group of chunks. The conversion from chunk number to region number is floor(coord
 * / 32): a chunk at (30, -3) would be in region (0, -1), and one at (70, -30) would be at (3, -1). Region files are
 * named "r.x.z.data", where x and z are the region coordinates.
 * </p>
 * <p>
 * A region file begins with a 4KB header that describes where chunks are stored in the file. A 4-byte big-endian
 * integer represents sector offsets and sector counts. The chunk offset for a chunk (x, z) begins at byte 4*(x+z*32) in
 * the file. The bottom byte of the chunk offset indicates the number of sectors the chunk takes up, and the top 3 bytes
 * represent the sector number of the chunk. Given a chunk offset o, the chunk data begins at byte 4096*(o/256) and
 * takes up at most 4096*(o%256) bytes. A chunk cannot exceed 1MB in size. If a chunk offset is 0, the corresponding
 * chunk is not stored in the region file.
 * </p>
 * <p>
 * Chunk data begins with a 4-byte big-endian integer representing the chunk data length in bytes, not counting the
 * length field. The length must be smaller than 4096 times the number of sectors. The next byte is a version field, to
 * allow backwards-compatible updates to how chunks are encoded.
 * </p>
 * <p>
...
public class RegionFile {
	...
	private final int region_x, region_z;
	...
	public RegionFile(File path) throws IOException {
		...
		String[] fparts = fileName.getName().split("\\.");
		region_x = Integer.parseInt(fparts[1]);
		region_z = Integer.parseInt(fparts[2]);
	 ...
	 * 
	 * @param x
	 *            The region-local chunk x coordinate.
	 * @param z
	 *            The region-local chunk z coordinate.
	 * @return A stream from which the (NBT) chunk data can be read.
	 * @throws IOException
	 *             If there is an error reading from the file or invalid data is encountered.
	 */
	public synchronized DataInputStream getChunkDataInputStream(int x, int z) throws IOException {
		...
		try {
			...
			if (offset == 0) {
				...
				throw new IOException("Chunk does not exist " + x + " " + z);
			}
			...
			if (sectorNumber + numSectors > sectorFree.size()) {
				throw new IOException("Invalid sector " + x + " " + z);
			}
			...
			if (length > SECTOR_BYTES * numSectors) {
				throw new IOException("Invalid length " + x + " " + z + ": " + length + " > 4096 * " + numSectors);
			}
			...
			throw new IOException("Unknown version " + x + " " + z);
		} catch (IOException e) {
			throw new IOException("Failed to read chunk at " + x + "," + z, e);
		}
	}
	...
	/** Write chunk data to the region file at specified sector number. */
	...
	public String toString() {
		return "RegionFile[" + fileName.getName() + "]";
	}
